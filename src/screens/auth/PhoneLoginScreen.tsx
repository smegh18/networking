...
const PhoneLoginScreen: React.FC<Props> = ({ navigation }) => {
  ...
  const handleSendOtp = useCallback(async () => {
    ...
    try {
      ...
      const registeredUser = await findUserProfileByPhone(normalizedPhone);
      if (!registeredUser) {
        ...
        Alert.alert(
          ...
          [
            { text: t('common.cancel', 'Cancel'), style: 'cancel' },
            {
              ...
              onPress: () => {
                // Go to email-based registration flow
                // (Login screen in register mode leads to Register after OTP).
                // We don't start phone auth for unregistered numbers.
                (navigation as any).navigate('Login', { mode: 'register' });
              },
            },
          ],
        );
  ...
  }, [navigation, normalizedPhone, phoneE164, t]);
  ...
  }, [otp, confirmationResult, navigation, phoneE164, t]);

  const showOtpStep = !!confirmationResult;

  return (
    <ScreenWrapper padded={false} scrollable={false}>
      <Header title={t('login.title', 'Login')} onBack={() => navigation.goBack()} />
